...
// 归一化 + 聚合：以分钟为粒度合并（同一分钟仅一条卡片）
function normalizeAndGroupAILogs(list) {
  const rows = (Array.isArray(list) ? list : []).map((raw) => {
    const timeStr = raw.time ?? raw.timestamp ?? '';
    const ts = timeStr ? new Date(timeStr).getTime() : 0;
    const minuteKey = timeStr ? new Date(timeStr).toISOString().slice(0, 16) : String(ts);
    return {
      time: timeStr,
      ts,
      minuteKey,
      analysis: toPlainText(raw.analysis ?? raw.message ?? '')
    };
  });
  ...
  for (const row of rows) {
    const key = row.minuteKey;
    if (!map.has(key)) {
      map.set(key, { time: row.time, ts: row.ts, analysis: row.analysis });
    } else {
...
async function loadAILogs() {
  try {
    ...
    if (!response.ok) throw new Error('网络错误');
    const data = await response.json();
    currentData.aiLogs = normalizeAndGroupAILogs(data);
  } catch (error) {
...
function updateAILogsList() {
  ...
  // 使用 DOM API 渲染，全部使用 textContent，避免任何嵌套HTML带来的“框中框”
  for (const log of currentData.aiLogs) {
    const item = document.createElement('div');
    item.className = 'ai-log-item';

    const timeDiv = document.createElement('div');
    timeDiv.className = 'ai-log-time';
    timeDiv.textContent = log.time || '';

    const textDiv = document.createElement('div');
    textDiv.className = 'ai-log-analysis';
    textDiv.textContent = log.analysis || '';
